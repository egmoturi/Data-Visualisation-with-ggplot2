...
```{r}
colSums(is.na(hotel_bookings_df))
```
#Removing missing Values or NA
```{r}

sum(is.na(hotel_bookings_df$children))
```


Record your general observations below:

Observation 1 The most common type of booking for hotels are Transient customers
Observation 2 Alot of these data is categorical are as character data and therefore do not have alot of information.
...
(colSums(is.na(hotel_bookings_df))  == ncol(hotel_bookings_df))
```
...
(rowSums(is.na(hotel_bookings_df)) == nrow(hotel_bookings_df))
```
...
# YOUR CODE GOES BELOW
# 
hotel_booking_df = select(hotel_bookings_df, -company)
head(hotel_bookings_df)
```
...
ggplot(hotel_booking_df, aes(x= reservation_status, y= is_canceled))+
  geom_col(stat = "identity", width =  0.5, fill= "green") +
  labs(
...
ggplot(data = stay_length, aes(x= hotel, y= total_stay_days))+
  geom_col(stat = "identity", width =  0.8, fill= "yellow") +
  labs(
...
There were too many aspects to explorefrom this dataset
